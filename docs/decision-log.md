...
## Milestone 23/24 implementation decisions (server-side ticket state)

M23 (provenance/visibility fields, `server/seed.ts`/`store.ts`/`visibility.ts` scaffolding)
was started in a session interrupted by a usage limit; M24 (the rest of this section)
completed it in the next session, then finished the migration in full.

| # | Decision | Rationale |
|---|---|---|
| D48 | **A standalone mock HeyQ server, in TypeScript** (not ported to JS), is now the sole owner of all ticket, notification, and order-provider state — not just cross-app tickets | For a shared backend to be trustworthy, HeyQ's browser can hold no authoritative copy of anything the server also serves. Keeping it TypeScript (not JS) avoids a lossy rewrite of ~800 lines of typed lifecycle logic and keeps one set of types shared between server and client. |
| D49 | **Pure, environment-independent modules (`slaService.ts`, `data/catalog.ts`, `lib/mock.ts`, `lib/clock.ts`) are imported directly by the server, not duplicated**; the server never loads a *browser-oriented service* module (`ticketService.ts` itself, `notificationService.ts`, etc.) | These modules have no DOM dependency and no mutable state the server would need to share live with the browser (Node and the browser are separate processes regardless). Reusing them avoids drift between two copies of the same SLA math and catalog data. Ticket/notification/order-provider *state* modules are a different class — mutable and meant to be singly-owned — so those were ported, not imported. |
| D50 | **Stores are keyed by an opaque `storeId` (default `'default'`, header `X-Store-Id`), with a generic `isDown`/`setDown` per store** replacing the old `businessPlusProviderState.available` flag | One mechanism for "this store is down" serves both the future multi-tenant-test case and the M22 outage simulation, instead of a bespoke boolean living in browser module state that the server could never see. |
| D51 | **Every client service (`ticketService`, `requesterService`, `reportsService`, the ticket half of `auditService`, `notificationService`, `orderProvider`) keeps its exact pre-M23 exported function signatures and types** — only the function bodies became `fetch` calls (`src/app/lib/apiClient.ts`) | UI components and nearly all existing tests needed zero edits; the migration is invisible above the service layer. `auditService`'s KB-derived entries stay local (KB isn't ticket state) and are merged client-side with the server's ticket-derived entries. |
| D52 | **The test API server starts fresh inside `src/test/setup.ts` (a `setupFiles` hook), not in `globalSetup`** | Vitest isolates modules **per test file** by default. A server started once in `globalSetup` runs in a separate module registry from any test file, so `adminService.ts` mutating `data/catalog.ts` (routing team, SLA targets, agent active state) in a test would be invisible to that separate server process — silently breaking the tests that check routing/SLA side effects on ticket creation. Starting the server inside each file's own `setupFiles` context keeps it in the *same* module registry as that file's other imports, restoring the pre-migration behavior where everything shared one module graph. This also gives each test file free store isolation (a fresh server + fresh in-memory store per file) with no store-id bookkeeping needed in `apiClient.ts`. |
| D53 | **`tsx` as a devDependency; `npm run server` / `npm run dev` stay two separate scripts** run in two terminals, no process-orchestration dependency | Confirmed with the user: avoids Node's `--experimental-strip-types` flag friction while keeping the dev workflow minimal — no `concurrently`/`npm-run-all` added for one more command. |
| D54 | **`data/tickets.ts` and `data/businessPlusOrders.ts` are kept, not deleted, as read-only test fixtures** (re-documented as such); `data/notifications.ts` was deleted | Three tests (`ticketFields.test.ts`'s seed-coverage assertions; `orderProvider.test.ts`'s `countFor` scoping check) still need static, known-good fixture data independent of the live server response, and `businessPlusOrders.ts` also supplies the client's `BusinessPlusOrderRecord`/`BusinessPlusOrg`/`BusinessPlusUser` types. `data/notifications.ts` had zero remaining consumers once `notificationService.ts` became a thin client, so it was removed outright rather than left as dead weight. |
